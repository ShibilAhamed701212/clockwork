...
def test_generate_agent_context_contains_project_name(gen):
    ctx = gen._load_context()
    content = gen.generate_agent_context(ctx, gen._load_rules(), {})
    assert "myapp" in content
    assert "FastAPI" in content


def test_generate_agent_context_contains_rules(gen):
    ctx = gen._load_context()
    rules = gen._load_rules()
    content = gen.generate_agent_context(ctx, rules, {})
    assert "API layer" in content


def test_generate_agent_context_has_tasks(gen):
    ctx = gen._load_context()
    content = gen.generate_agent_context(ctx, gen._load_rules(), {})
